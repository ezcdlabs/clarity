...
When the current week has no deploys the header carries no summary at all, exactly as `HEAD` and `CI Passed` render empty when they are. It deliberately does not say "no deploys yet this week": a flow that releases every three weeks would carry that banner two weeks in three, which is a permanent notice of the normal condition, and the `deployed 4d ago` subheader immediately below already answers the question without editorialising. "Deploys" counts distinct deploy batches (not per-commit events) and "avg" averages the per-commit lead times shown on the right of each row — see [Lead time](#lead-time) for what those measure and how to change it. ISO weeks are computed in UTC so the same data renders into the same buckets regardless of the user's locale.
